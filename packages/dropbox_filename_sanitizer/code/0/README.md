...
Nimble package manager <https://github.com/nim-lang/nimble>`_ to install the
binary::

    $ nimble update
    $ nimble install argument_parser
    $ nimble install dropbox_filename_sanitizer

This will install the ``dropbox_filename_sanitizer`` command into Nimble's
binary directory.
...
usage. You can also read the files installed with the Nimble package and use the
``doc`` `nakefile task <https://github.com/fowlmouth/nake>`_ to build their
HTML version. Unix example::

    $ cd `nimble path dropbox_filename_sanitizer`
    $ nake doc
    $ open docindex.html
