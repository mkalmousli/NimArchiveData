...
To install this package you need to download the repository via git. As far as I know, repositories on codeberg don't make it to the official nimble package list.

Once downloaded, you only need to cd to the directory and type:
```
nimble install
```
or
...
links and author are handled differently in Atom and RSS. links are included as an attribute in Atom and as text in RSS. Users should be mindful of this and also regarding users. To avoid more problems RSS objects that deal with RSS have an additional only add *name* to the Author object, whereas Atom additionally add *email* and *uri* if available.
