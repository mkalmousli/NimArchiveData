...
The Claude SDK for Nim allows you to interact with Claude Code programmatically, supporting both simple one-shot queries and complex interactive conversations. This SDK maintains API compatibility with the Python version while leveraging Nim's type safety and performance benefits.
...
```nim
requires "claude_code_sdk_nim >= 0.0.19"
```

Or install directly:

```bash
nimble install claude_code_sdk_nim
```
...
proc interactiveExample() {.async.} =
  ...
  try:
    ...
    # Client would normally stream responses here
    # (Full async implementation available for real CLI integration)
    
  finally:
    client.disconnect()

waitFor(interactiveExample())
```
...
```nim
# Basic query
for message in query("Explain recursion"):
  echo message

# With options
var opts = newClaudeCodeOptions()
opts.cwd = some("/my/project")
for message in query("Analyze this codebase", opts):
  echo message
```
